...
def create_tracking_plots_avg(file_names, algo_name, outputFolder):
    ...
    Plots are saved as 'tracking_error_vs_time.png' and 'tracking_path_xy.png'.
    '''
    algo_colors = {"NaiveP":"red","Waypoint":"darkorange","Breadcrumbs":"green","BreadcrumbsTimedPop":"blue","HeadSwivel":"purple"}
    # Load the dataset
    df = []
    for i in range(len(file_names)):
        try:
    ...
    for j in range(len(df)):
        ...
        if 'target_acquired' in dftemp.columns:
            ...
            # If column is missing, append NaN or 0 so lists stay aligned
            all_loss_counts.append(0) # Or np.nan
    ...
    try:
        ...
        plt.ylabel('Euclidean Distance (m)',fontsize=16)
    ...
    try:
        ...
        for i in range(len(df)):
        	ax.plot(df[i]['pepper_x'], df[i]['pepper_y'], color=algo_colors[algo_name], alpha=0.7, linestyle='--')	
...
def create_tracking_plots_avg_compare(file_names,algo_names):
    ...
    for j in range(len(file_names)):
        ...
        for k in range(len(df)): # Renamed inner loop variable
        	df[k]['error_xy'] = np.sqrt((df[k]['person_x'] - df[k]['pepper_x'])**2 + (df[k]['person_y'] - df[k]['pepper_y'])**2)
        	dfAvg['person_x'] = dfAvg['person_x'] + df[k]['person_x']
        	dfAvg['person_y'] = dfAvg['person_y'] + df[k]['person_y']
        	dfAvg['pepper_x'] = dfAvg['pepper_x'] + df[k]['pepper_x']
        	dfAvg['pepper_y'] = dfAvg['pepper_y'] + df[k]['pepper_y']
        	dfAvg['error_xy'] = dfAvg['error_xy'] + df[k]['error_xy']
        dfAvg['person_x'] = dfAvg['person_x'] / len(df)
        dfAvg['person_y'] = dfAvg['person_y'] / len(df)
        dfAvg['pepper_x'] = dfAvg['pepper_x'] / len(df)
        dfAvg['pepper_y'] = dfAvg['pepper_y'] / len(df)
    ...
    try:
        ...
        for i in range(len(dfAvgList)):
        	upper_bound = dfAvgList[i]['error_xy'] + dfAvgList[i]['error_xy_std']
        	lower_bound = dfAvgList[i]['error_xy'] - dfAvgList[i]['error_xy_std']	
        	plt.plot(dfAvgList[i]['timestamp'], (dfAvgList[i]['error_xy']).cumsum(),color=algo_colors[algo_names[i]],label=f'{algo_names[i]} Mean')
        	#plt.fill_between(dfAvgList[i]['timestamp'], lower_bound, upper_bound, color=algo_colors[algo_names[i]], alpha=0.1, label=f'{algo_names[i]} SD')	
        plt.xlabel('Time (s)',fontsize=16)
        plt.ylabel('Cumulative Euclidean Distance (m)',fontsize=16)
