...
def create_tracking_plots_avg(file_names, algo_name, outputFolder):
    ...
    try:
        ...
        plt.plot(dfAvg['pepper_x'], dfAvg['pepper_y'], label=f'{algo_name}', color=algo_colors[algo_name], linestyle='-')
        for i in range(len(df)):
...
def create_tracking_plots_avg_compare(file_names,algo_names):
    ...
    try:
        ...
        for i in range(len(dfAvgList)):
        	...
        	plt.plot(dfAvgList[i]['timestamp'], dfAvgList[i]['error_xy'],color=algo_colors[algo_names[i]],label=f'{algo_names[i]} Mean')
