...
class PepperFollower:
    def __init__(self):
        ...
        # --- 2. Load Detector and Re-ID Model ---
        ...
        # --- State Machine and Search Logic ---
        self.robot_state = 'SEARCHING' # Start in SEARCHING state
        self.last_target_seen_time = rospy.Time.now()
        self.LOST_TIMEOUT = rospy.Duration(2.0)
        self.SEARCH_SPEED = 0.3

        # --- 3. NEW: Shared State Variables ---
        # These variables are written by perception_callback and read by control_loop
        self.target_visible = False
        self.target_error_x = 0.0     # Replaces last_known_error_x
        self.target_distance = 0.0    # Current distance to target
        self.depth_frame_nan = False  # Flag to check for bad depth readings
        # ---
        ...
        # --- 4. MODIFIED: Register the correct callback ---
        self.ts.registerCallback(self.perception_callback)
        # ---

    # --- 5. REPLACED: get_masked_hue_hist is replaced by get_reid_features ---
    @torch.no_grad()
    ...
    def control_loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.target_visible:
                ...
                self.last_target_seen_time = rospy.Time.now() # Update time
                status_msg = Bool()
                status_msg.data = True
                self.target_status_pub.publish(status_msg)
            else:
                # Target is not visible in this perception frame
                if self.robot_state == 'TRACKING':
                    # We were tracking, check if we've lost them
            ...
            if self.robot_state == 'TRACKING':
                ...
                # 2. Linear (Distance) Control
                if self.target_distance > 0: # We have a valid distance
                    ...
                    # Target is visible but depth is bad, don't move forward/back
                    rospy.logwarn_throttle(1.0, "Target visible but depth is NaN. Stopping linear motion.")
                    move_cmd.linear.x = 0.0

            elif self.robot_state == 'SEARCHING':
                # Use the *last known* horizontal error to decide which way to turn
                if self.target_error_x > 0: # Last seen to the right
                    move_cmd.angular.z = -self.SEARCH_SPEED # Turn right
                else: # Last seen to the left (or center)
                    move_cmd.angular.z = self.SEARCH_SPEED # Turn left
            
            # Publish the command (even if it's all zeros)
    ...
    # --- 7. MODIFIED: Renamed and Refactored Perception Callback ---
    def perception_callback(self, color_msg, depth_msg):
        ...
        try:
            ...
            # Use 32FC1 for depth as in your original file, checks for NaN
        ...
        if results[0].boxes.id is not None:
            for i, box in enumerate(results[0].boxes.xyxy):
                ...
                # Use results[0].boxes.id to get the track ID
        ...
        if crops_to_process:
            ...
            for i, candidate in enumerate(candidates):
                ...
                if 0 in self.known_people:
                    # --- We have a target (ID 0), try to find it ---
                    ...
                    # --- We don't have a target, try to acquire one ---
        ...
        # --- 9. MODIFIED: Update shared variables, do not send commands ---
        if target_bbox is not None:
            self.target_visible = True
            
            x1, y1, x2, y2 = target_bbox
            
            # Update horizontal error
            ...
            if 0 <= depth_coord_y < depth_h and 0 <= depth_coord_x < depth_w:
                ...
                # BBox center is outside depth frame, invalid
                self.target_distance = 0.0
                self.depth_frame_nan = True
        else:
            # Target is not visible in this frame
            self.target_visible = False
            self.target_distance = 0.0
        
        # All cv2.imshow/waitKey logic is removed, as it's not part of
        # the core logic and can block the callback.
    # ---

    # --- 10. MODIFIED: run() function to start the thread ---
    def run(self):
        # Start the control_loop in a separate thread
        control_thread = Thread(target=self.control_loop)
        control_thread.daemon = True # Allows thread to exit when main program exits
        control_thread.start()

        rospy.spin() # Keep the main thread alive for callbacks

        # This will be called on shutdown (e.g., Ctrl+C)
        self.cmd_pub.publish(Twist()) # Send a stop command
